...
class ChatService:
    ...
    async def chat(
        ...
        logger.info(
            "chat_workflow_construction_begin",
            extra={"event": "chat_workflow_construction_begin"},
        )
        with closing(self._session_factory()) as db_session:
            ...
            if getattr(workflow, "meal_service", meal_service) is None:
                logger.error(
                    "chat_workflow_meal_service_missing",
                    extra={"event": "chat_workflow_meal_service_missing"},
                )
                raise RuntimeError("TiffinCatalogService is required for chat workflow execution.")
