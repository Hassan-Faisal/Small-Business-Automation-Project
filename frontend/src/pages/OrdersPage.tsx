...
const ACTIONS: Record<string, Array<{ status: OrderStatus; label: string; requiresConfirmation: boolean }>> = {
  draft: [{ status: "confirmed", label: "Confirm order", requiresConfirmation: false }, { status: "cancelled", label: "Cancel order", requiresConfirmation: true }],
...
export function OrdersPage() {
  ...
  useEffect(() => {
    ...
    listOrders(filters, page)
      ...
      .catch((reason: unknown) => {
        ...
        else setLoadError(getApiErrorMessage(reason, "We could not load orders right now. Please try again."));
      })
  ...
  return <div>
    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
      <div><p className="text-sm font-semibold uppercase tracking-widest text-teal-600">Operations</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-ink">Orders</h1><p className="mt-2 text-sm text-slate-500">Review customer orders and keep every delivery moving.</p></div>
    ...
    <section aria-label="Order filters" className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <FilterInput id="order-search" label="Search" value={filters.search} placeholder="Order or phone" onChange={(value) => changeFilter("search", value)} />
        <FilterInput id="order-number" label="Order number" value={filters.order_number} placeholder="ORD-..." onChange={(value) => changeFilter("order_number", value)} />
        <FilterInput id="customer-phone" label="Customer phone" value={filters.customer_phone} placeholder="+92..." onChange={(value) => changeFilter("customer_phone", value)} />
    ...
    {action && <ConfirmDialog title={action.status === "cancelled" ? "Cancel this order?" : "Complete this order?"} message={action.status === "cancelled" ? "This order will be cancelled and cannot be reopened." : "Mark this order as completed? This confirms delivery is finished."} confirmLabel={action.label} confirming={saving} error="" onConfirm={() => void saveStatus()} onClose={() => { if (!saving) setAction(null); }} />}
...
function OrdersTable({ orders, onOpen }: { orders: OrderListItem[]; onOpen: (id: number) => void }) {
  return <><div className="hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm md:block"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-4">Order</th><th className="px-5 py-4">Customer</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Total</th><th className="px-5 py-4">Items</th><th className="px-5 py-4">Created</th><th className="px-5 py-4">Provider</th></tr></thead><tbody className="divide-y divide-slate-100">{orders.map((order) => <tr key={order.id} className="hover:bg-slate-50"><td className="px-5 py-4"><button type="button" className="font-semibold text-teal-700 underline-offset-2 hover:underline focus:outline-none focus:ring-4 focus:ring-teal-100" onClick={() => onOpen(order.id)}>{order.order_number}</button></td><td className="px-5 py-4 text-slate-600">{order.customer_phone}</td><td className="px-5 py-4"><StatusBadge status={order.status} /></td><td className="whitespace-nowrap px-5 py-4 font-medium text-ink">{formatMoney(order.total_amount)}</td><td className="px-5 py-4">{order.item_count}</td><td className="whitespace-nowrap px-5 py-4 text-slate-500">{formatOrderDate(order.created_at)}</td><td className="px-5 py-4 capitalize">{order.delivery_provider || "Not assigned"}</td></tr>)}</tbody></table></div><div className="grid gap-4 md:hidden">{orders.map((order) => <button key={order.id} type="button" className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm focus:outline-none focus:ring-4 focus:ring-teal-100" onClick={() => onOpen(order.id)}><div className="flex items-start justify-between gap-3"><span className="font-bold text-teal-700">{order.order_number}</span><StatusBadge status={order.status} /></div><p className="mt-3 text-sm text-slate-600">{order.customer_phone}</p><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><span><strong className="block text-xs uppercase tracking-wide text-slate-400">Total</strong>{formatMoney(order.total_amount)}</span><span><strong className="block text-xs uppercase tracking-wide text-slate-400">Created</strong>{formatOrderDate(order.created_at)}</span><span><strong className="block text-xs uppercase tracking-wide text-slate-400">Items</strong>{order.item_count}</span><span><strong className="block text-xs uppercase tracking-wide text-slate-400">Provider</strong><span className="capitalize">{order.delivery_provider || "Not assigned"}</span></span></div></button>)}</div></>;
}
...
function EmptyOrders({ filtered, onReset }: { filtered: boolean; onReset: () => void }) {
  return <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm"><h2 className="text-xl font-bold text-ink">{filtered ? "No matching orders" : "No orders yet"}</h2><p className="mt-2 text-sm text-slate-500">{filtered ? "Try different filters or reset the current search." : "New customer orders will appear here."}</p>{filtered && <button type="button" className="mt-5 rounded-xl bg-teal-600 px-5 py-3 text-sm font-semibold text-white hover:bg-teal-700" onClick={onReset}>Reset filters</button>}</div>;
}
